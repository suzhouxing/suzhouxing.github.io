function getWorkerNum() {
  return data.input.workers.names.length;
}
function getWorkerName(worker) {
  return data.input.workers.names[worker];
}
function getJobNum() {
  return data.input.jobs.names.length;
}
function getJobName(job) {
  return data.input.jobs.names[job];
}
function getWorker(job) {
  return data.output.assignments[job];
}
function getBeginTime(job) {
  return data.output.beginTimes[job];
}
function getDuration(job, worker = getWorker(job)) {
  return data.input.jobs.exes[job][worker] || Object.values(data.input.jobs.exes[job])[0];
}
function getEndTime(job, worker = getWorker(job)) {
  return getBeginTime(job) + getDuration(job, worker);
}
function getMakespan() {
  return data.output.makespan;
}

function isValidAssignment(job, worker = getWorker(job)) {
  return data.input.jobs.exes[job][worker] != undefined;
}
