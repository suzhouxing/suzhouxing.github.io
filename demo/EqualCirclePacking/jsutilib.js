...
function onDomReady(f) {
  let fresh = true;
  function run() { if (fresh) { fresh = false; f(); } }
...
function sCls(className) { return document.getElementsByClassName(className); }
function sTag(tagName) { return document.getElementsByTagName(tagName); }
...
function powSum(a, b) { return a * a + b * b; }

function updateMin(min, num, update) { if (min > num) { update(num); } }
function updateMax(max, num, update) { if (max < num) { update(num); } }

// svg.
const SvgNS = "http://www.w3.org/2000/svg";

function svgAdd(root, tag, attrs, text) {
  var e = document.createElementNS(SvgNS, tag);
  for (var a in attrs) {
    if (attrs.hasOwnProperty(a)) { e.setAttribute(a, attrs[a]); }
  }
